...
#随机梯度下降算法：每次迭代只只计算一个样本点
#w=w-xT*(x*w-y)*alpha 矩阵乘法
#x[i]:自变量. 行数：1个样本数，列数：特征数+1，加1是因为截距
#y[i]:因变量. 行数：1个样本数，列数：1
#w:回归系数. 行数：特征数+1，列数：1
#alpha：步长. 1*1，标量
def randGrad(x,y,n=1000,alpha=0.0001):
    n_features = x.shape[1] #包括截距
    n_samples = x.shape[0]
    w = np.mat(np.ones((n_features,1))) #回归系数初始化为1
    for j in range(n):
        dataIndex = range(n_samples) #每次迭代随机选取样本点
        samples = np.random.choice(dataIndex,len(dataIndex),False)
        for i in samples:
            w = w-x[i].transpose()*(x[i]*w-y[i])*alpha
    return w
